...
import { useHotkey, useUIState, InfoIcon, OrgSelect, IconBtn, ProfileIcon, TeamMenu, ProjectMenu, useClickOutside } from '../../global';
...
const SideBar = () => {
  ...
  return (
    ...
    <div className='h-[56px] border-b-2 border-zinc-500/10 bg-zinc-50 dark:bg-[#1c1c1c] flex items-center smooth'>
     ...
     <div ref={sidebarRef} className={`fixed top-0 left-0 h-screen w-62 bg-zinc-50 dark:bg-[#1c1c1c] border-r-2 border-zinc-500/10
       ...
       <div className="flex flex-col h-full justify-between">
         ...
         <div className="flex-1 overflow-y-auto hide-scrollbar p-2">
           <div className='flex flex-col gap-1 w-full text-[14px] font-medium tracking-tight dark:font-normal'>
              <Link className={`${location.pathname.includes('invite') ? "bg-zinc-800 text-zinc-300 shadow-md dark:bg-zinc-200 dark:text-zinc-800" : "hover:bg-zinc-500/20 "} text-zinc-500 rounded text-xs p-2 tracking-wider`} to={'/invite'}>
                 <IconBtn  text='Invitation' icon={<FiInbox size={16} className='mt-0.5'/>}/>
              </Link>
              <IconBtn className='text-xs text-zinc-500 font-medium p-2 tracking-wider' text='WorkForce' icon={< PiUsersFour size={16}/>}/>
