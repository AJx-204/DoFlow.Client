  ...
  return (
    <div className='relative max-w-[500px] h-screen bg-white shadow-lg dark:bg-zinc-900 overflow-auto flex flex-col gap-5 font-medium dark:font-normal hide-scrollbar'>
      <div className='flex justify-between items-center p-2.5'>
         <i onClick={()=>setShowTaskComments(false)} className='p-1.5 hover:shadow-md rounded-full hover:text-blue-500 cursor-pointer smooth text-gary-500'><MdClose size={20}/></i>
      </div>
      <div className='px-6'>
      ...
      <span className='px-6 text-sm flex gap-2 items-center text-gray-500'><PiChatsCircle size={18}/> Comments</span>
      <div className='px-6 flex flex-col gap-3 text-sm w-full'>
       {task?.comments?.map((c)=>(
          <div key={c?._id} className={`${c?.addedBy?._id == user._id ? "justify-end" : "justify-start "} flex `}>
             <div className={`${c?.addedBy?._id == user._id ? "bg-blue-900 text-white" : "border border-gray-500/50"} max-w-[70%] flex gap-3 items-center px-3 py-1.5 rounded-md `}>
               <div className='flex flex-col items-start'>
                  <p>{c?.text}</p>
                  <span className='text-[9px] opacity-50 font-normal'>{formatDate(c?.addedAt)}</span>
               </div>
               <img className='h-6 w-6 rounded-full' src={c?.addedBy?.profilePhoto} alt="" />
             </div>
          </div>
        ))}
      </div>
      <div className='flex gap-2 items-center absolute bottom-2 px-6 w-full'>
         <Input
           ...
           />
          <IconBtn className='rounded-md p-2 text-white bg-blue-500 shadow-md hover:bg-blue-700' text='' icon={<RiSendPlaneFill size={20}/>}/>
      </div>
    </div>
  )
}
