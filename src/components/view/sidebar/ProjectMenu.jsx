   ...
   <div className='flex flex-col gap-1'>
      <div className='flex items-center cursor-pointer justify-between text-xs text-zinc-500 font-medium rounded p-2 hover:bg-zinc-500/20 group smooth'>
        <IconBtn 
         ...
         />
         <IoMdAdd size={16} className='text-zinc-900 dark:text-zinc-200 opacity-40 group-hover:opacity-100 smooth'/>
       </div>
       {org?.projects?.length > 0 ? (
        org.projects.map((p)=>(
          <Link onClick={()=> dispatch(setProject(p))} to={`/${org.orgName}/${p?.projectName}/${p?._id}/project`} className='flex ' key={p._id}>
            <IconBtn 
