...
import { Btn, IconBtn, useClickOutside, useLogout, useTheme } from '../../../global';
     ...
     >
      ...
      <div className='px-2 py-1 flex flex-col gap-1'>
        <Link to={`/profile/${user.userName}`}>
          <IconBtn
           icon={<Settings className='text-zinc-500' strokeWidth={1.5} size={18}/>}
        ...
        <IconBtn
         ...
         icon={<Palette className='text-zinc-500' strokeWidth={1.5} size={18}/>}
         text='Theme'
      ...
      </div>
      {
        showThemeBtn && (
          <div className='absolute -right-40 top-25 p-2 flex gap-2 bg-white dark:bg-zinc-950 border rounded-md border-zinc-500/30 shadow-md smooth'>
              <Btn onClick={()=>lightTheme()} className='bg-white px-4 py-2 rounded-md text-black border border-black shadow-md' text='Light'/>
              <Btn onClick={()=>darkTheme()} className='bg-black px-4 py-2 rounded-md text-white border border-white shadow-md' text='Dark'/>
          </div>
        )
      }
    </div>
