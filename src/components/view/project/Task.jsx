...
import { AvatarGroup, Comments, formatDate, GetPriorityColor, IconBtn,  useUIState } from '@/global'
...
const Task = ({}) => {
  ...
  const { setShowTaskPage, showTaskComments, setShowTaskComments } = useUIState();
  ...
  return (
    <div className='relative max-w-[500px] h-screen bg-white shadow-lg dark:bg-zinc-900 overflow-auto flex flex-col gap-5 font-medium dark:font-normal hide-scrollbar'>
        <div className='flex justify-between items-center p-2.5'>
           <i onClick={()=>setShowTaskPage(false)} className='p-1.5 hover:shadow-md rounded-full hover:text-blue-500 cursor-pointer smooth text-gary-500'><MdClose size={20}/></i>
           <i className='p-1.5 hover:shadow-md rounded-full hover:text-blue-500 cursor-pointer smooth text-gary-500'><CgMenuRightAlt size={20}/></i>
        </div>
        <div className='px-6'>
          ...
          <span className='px-6 text-sm flex gap-2 items-center'><BiTask size={18}/> SubTask</span>
             ...
             {task?.subTasks?.map((sub)=>(
               <div key={sub?._id} className='flex gap-6 items-center justify-between'>
                ...
                <div className='flex items-center gap-2 text-xs '>
                   Add By 
                   <img className='h-5 w-5 rounded-full' src={sub?.addedBy?.profilePhoto} alt="" />
                </div>
               </div>
             ))}
          </div>
          <span className='px-6 text-sm flex gap-2 items-center'><Link size={16} className='mt-1'/>Links</span>
          <div className='flex flex-col text-gray-500  gap-2 pl-10 pr-3 border-b-2 border-gray-500/10 pb-6'>
             {task?.links?.map((link)=>(
               <div key={link?._id} className='flex gap-6 items-center justify-between '>
                <a href={link?.url} target='_blank' className='flex gap-3 items-center hover:underline hover:text-blue-500 smooth'>
                  < LuSquareArrowOutUpRight size={14} className=' mt-0.5'/>
                  <span className='text-sm'>{link?.label}</span>
                </a>
                <div className='flex items-center gap-2 text-xs '>
                    Add By 
                   <img className='h-5 w-5 rounded-full' src={link?.addedBy?.profilePhoto} alt="" />
                </div>
               </div>
             ))}
          </div>
          <span className='px-6  text-sm flex gap-2 items-center'><Paperclip size={16} className='mt-1'/>Attachment</span>
          <div className='pl-10 text-gray-500 pr-3 flex flex-col gap-2 border-b-2 border-gray-500/10 pb-6'>
             {task?.attachments?.map((a)=>(
               <div key={a?._id} className='flex gap-6 items-center justify-between '>
                <a href={a?.fileUrl} target='_blank' className='flex gap-3 items-center hover:underline hover:text-blue-500  smooth'>
                  <TiAttachment size={18} className='mt-0.5'/>
                  <span className='text-sm line-clamp-1'>{a?.fileName}</span>
                </a>
                <div className='flex items-center gap-2 text-xs text-gray-500'>
                    Add By 
                   <img className='h-5 w-5 rounded-full' src={a?.addedBy?.profilePhoto} alt="" />
                </div>
               </div>
             ))}
          </div>
          ...
          />
          <div className={`fixed z-100 top-0 right-0 ${showTaskComments ? 'translate-x-0' : 'translate-x-full'} transition-all duration-400 ease-in`}>
             <Comments/>
          </div>
    </div>
  )
