import { AvatarGroup, useUIState, GetPriorityColor, setTask, useClickOutside, Task } from '@/global';
...
const ProjectSection = () => {
  ...
  const [expandedSections, setExpandedSections] = useState(statusOrder.reduce((acc, status) => {
    acc[status] = true;
    return acc;
  }, {}));
   ...
   taskShowInBoard ? (
    <>
      ...
      <div className="overflow-x-auto">
          ...
          <div key={status} className="pb-6">
            <div className="overflow-x-auto">
              <table className="min-w-[800px] w-full text-sm text-left">
                <thead className={`text-zinc-600 dark:text-zinc-400 capitalize border-y-2 border-zinc-500/6 inter 
                  ${statusColor[status]}
                `}>
                  <tr>
                    <th onClick={()=>toggleSection(status)} className='font-medium text-gray-500/70 hover:text-blue-500 pl-3 w-[35px] cursor-pointer'>
                ...
                {expandedSections[status] &&<tbody>
                  {tasks.length > 0 ? (
                    tasks.map(task => (
                      <tr onClick={()=>(dispatch(setTask(task)), setShowTaskPage(true))} key={task._id} className="hover:bg-zinc-500/5 border-b cursor-pointer border-gray-500/10">
                        ...
                        <td className={`${GetPriorityColor(task.priority)} text-xs font-medium dark:font-normal p-4 border-r border-gray-500/10`}>
                          {task.priority || '—'}
                        </td>
        ...
        <div ref={taskRef} className={`fixed z-90 top-0 right-0 ${showTaskPage ? 'translate-x-0' : 'translate-x-full'} transition-all duration-400 ease-in`}>
           <Task/>
        </div>
      </div>
    )
