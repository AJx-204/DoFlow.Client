...
const { messages, removeMessage, pauseTimer, resumeTimer } = useNotifier();
  ...
  return (
    <div className="fixed bottom-5 left-5 flex flex-col space-y-2 z-200 w-[250px] ">
      {messages.map(({ id, text, type }) => (
        <div
          key={id}
          className={`text-sm flex items-center group justify-between p-3 pr-5 rounded-lg shadow-md border ${toastColors[type] || toastColors.info} animate-slideIn`}
          role="alert"
