...
import { useNotifier, setOrg, setOrgLoading, setOrgErrorMessage, setUser, setMemberLoading, setMemberErrorMessage, setInviteLoading, setInviteErrorMessage} from "@/global";
...
const useOrg = () => {
    ...
    const createOrg = async (data, orgId) => {
        ...
        try {
            ...
            dispatch(setOrgErrorMessage(error.response?.data?.message || 'failed to create organization !'))
            return false;
        } finally{
            dispatch(setOrgLoading(false))
        }
    ...
    const UpdateOrg = async (data, orgId) => {
        ...
        try {
            ...
            dispatch(setOrgErrorMessage(error.response?.data?.message || 'failed to Update organization !'))
            return false;
        } finally{
            dispatch(setOrgLoading(false))
        }
    ...
    const DeleteOrg = async (orgId, orgName) => {
        ...
        try {
            ...
            dispatch(setOrgErrorMessage(error.response?.data?.message || 'failed to DeleteOrg organization !'))
            addMessage(error.response?.data?.message , 'error' || 'failed to delete organization !', 'error')
            return false;
        } finally{
            dispatch(setOrgLoading(false))
        }
    ...
    const inviteMemberToOrg = async (data) => {
        dispatch(setOrgLoading(true))
        try {
            const res = await axios.post(`${backendUrl}/org/inviteMember/${org._id}`,
                data ,{
                withCredentials:true
            })
            if(res){
            ...
            dispatch(setOrgErrorMessage(error.response?.data?.message || 'failed to invite Member !'))
    ...
    const changeOrgMemberRole = async (data, memberId) => {
        ...
        try {
            const res = await axios.patch(`${backendUrl}/org/${org._id}/changeOrgMemberRole/${memberId}`,
                data ,{
                withCredentials:true
            })
            if(res.data.data){
    ...
    const removeMemberFromOrg  = async (memberId) => {
        dispatch(setMemberLoading(true))
        try {
            const res = await axios.delete(`${backendUrl}/org/${org._id}/removeMemberFromOrg/${memberId}`,
            {
                withCredentials:true
            })
            if(res){
                const newMembers = org.members.filter(
    ...
    const transferOwnershipOfOrg  = async (memberId) => {
        dispatch(setMemberLoading(true))
        try {
            const res = await axios.patch(`${backendUrl}/org/${org._id}/transferOwnershipOfOrg/${memberId}`,{},
            {
                withCredentials:true
            })
            if(res.data){
    ...
    const acceptInviteForOrg  = async (inviteId) => {
        ...
        try {
            const res = await axios.patch(`${backendUrl}/org/acceptInviteForOrg/${inviteId}`,{},
            {
                withCredentials:true
            })
            if(res.data.data){
                const newInvitation = user.invites.filter((i)=>
                 i = i._id != inviteId
                );
            ...
            dispatch(setInviteErrorMessage(error.response?.data?.message || 'failed to accepte Invitation !'))
            return false;
        } finally{
    ...
    const declineInviteForOrg = async (inviteId) => {
        dispatch(setInviteLoading(true))
        try {
            const res = await axios.delete(`${backendUrl}/org/declineInviteForOrg/${inviteId}`,
            {
                withCredentials:true
            })
            if(res){
      ...
      const leaveOrg  = async (orgId) => {
        ...
        try {
            ...
            if(res){
                const newOrg = user.inOrg.filter(
                  (o) => o.org._id !== orgId
                );
                dispatch(setUser({
                  ...user,
            ...
            dispatch(setOrgErrorMessage(error.response?.data?.message || 'failed to leaved organization !'))
            return false;
        } finally{
            dispatch(setOrgLoading(false))
        }
